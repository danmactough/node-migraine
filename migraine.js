...
function performMigration(direction, migrationName) {
  ...
  set[direction](onNext.bind(set, direction), migrationPath);
}

function saveErrorPosition (direction) {
  function positionOfMigration() {
    for(var i=0; i < this.migrations.length; ++i) {
      if (this.migrations[i].title == currentMigration.title) {
        return 'up' === direction ? i : (this.pos - i);
      }
    }
    return -1;  
  }
  this.pos = positionOfMigration.call(this);
  var json = JSON.stringify(this);
  fs.writeFileSync(this.path, json);
}

function onNext (direction, err) {
  if (err) {
    console.error(err);
    saveErrorPosition.call(this, direction);
  }
